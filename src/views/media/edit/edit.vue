...
import { addDoc, getDoc, modDoc, getResGroups } from "@/api/doc";
export default {
  name: "media.edit",
  ...
  computed: {
    title() {
      return this.doc_id ? "编辑图文消息" : "新增图文消息";
    },
  },
  created() {
    this.getResGroups();
    if (this.$route.params.doc_id) {
      ...
      this.$router.push({ path: "/media/doc" });
    }
  },
  ...
  methods: {
    ...
    getDoc() {
      getDoc({ doc_id: this.doc_id }).then(
        (res) => {
          if (res.code == 0) {
            this.docData = res.data;
            this.$refs.quill.setContent(res.data.content);
          }
        },
        (err) => {
          console.log(err);
